      ...
      {FEATURES.map(({ icon: Icon, title, tagline, desc, color, gradient, border, glow, bullets }, i) => {
        ...
        return (
          <motion.div
            ...
            className={`relative overflow-hidden rounded-2xl cursor-pointer transition-all duration-300 shadow-neu-inset bg-background/50 glass-premium`}
            style={{
              backdropFilter: 'blur(16px)',
              border: `1px solid ${isOpen ? border : 'var(--border)'}`,
              boxShadow: isOpen
                ? `0 0 40px ${glow}, inset 0 2px 12px rgba(0,0,0,0.4)`
                : 'inset 0 2px 12px rgba(0,0,0,0.2)',
            }}
            whileHover={{
              borderColor: border,
              boxShadow: `0 0 20px ${glow}, inset 0 4px 16px rgba(0,0,0,0.3)`,
            }}
